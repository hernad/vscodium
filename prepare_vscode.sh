...
updateUrl='setpath(["updateUrl"]; "http://download.bring.out.ba/eShell")'
...
nameShort='setpath(["nameShort"]; "eShell")'
nameLong='setpath(["nameLong"]; "eShell")'
linuxIconName='setpath(["linuxIconName"]; "eshell")'
applicationName='setpath(["applicationName"]; "eshell")'
win32MutexName='setpath(["win32MutexName"]; "eshell")'
win32DirName='setpath(["win32DirName"]; "eShell")'
win32NameVersion='setpath(["win32NameVersion"]; "eShell")'
win32RegValueName='setpath(["win32RegValueName"]; "eShell")'
win32AppUserModelId='setpath(["win32AppUserModelId"]; "Microsoft.eShell")'
win32ShellNameShort='setpath(["win32ShellNameShort"]; "eShell")'
win32x64UserAppId='setpath (["win32x64UserAppId"]; "{{2E1F05D1-C245-4562-81E0-28188DB6FD18}")'
urlProtocol='setpath(["urlProtocol"]; "eshell")'
...
serverDataFolderName='setpath(["serverDataFolderName"]; ".eShell-server")'
dataFolderName='setpath(["dataFolderName"]; ".eShell")'
reportIssueUrl='setpath(["reportIssueUrl"]; "https://github.com/hernad/vscodium/issues/new/choose")'
licenseUrl='setpath(["licenseUrl"]; "https://github.com/hernad/vscodium/blob/eShell/LICENSE")'

product_json_changes="${checksumFailMoreInfoUrl} | ${tipsAndTricksUrl} | ${twitterUrl} | ${requestFeatureUrl} | ${documentationUrl} | ${introductoryVideosUrl} | ${extensionAllowedBadgeProviders} | ${updateUrl} | ${releaseNotesUrl} | ${keyboardShortcutsUrlMac} | ${keyboardShortcutsUrlLinux} | ${keyboardShortcutsUrlWin} | ${quality} | ${extensionsGallery} | ${linkProtectionTrustedDomains} | ${nameShort} | ${nameLong} | ${linuxIconName} | ${applicationName} | ${win32MutexName} | ${win32DirName} | ${win32NameVersion} | ${win32RegValueName} | ${win32AppUserModelId} | ${win32ShellNameShort} | ${win32x64UserAppId} | ${urlProtocol} | ${extensionAllowedProposedApi} | ${serverDataFolderName} | ${dataFolderName} | ${reportIssueUrl} | ${licenseUrl}"
